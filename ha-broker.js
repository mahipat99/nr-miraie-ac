...
const publishDisoveryMessages = () => {
    discoveredDevices.map(d => {
       //const msg = mqttDisco.generateDiscoMessage(d); //remove
       //publishMessage(msg.topic, msg.payload);   //remove
    });
};

const onConnected = () => {
    Logger.logInfo('HA broker connected.');
    //publishDisoveryMessages();  //remove
...
function HABroker(devices, onCmdReceieved) {
    ...
    //mqttDisco = new Discovery(); //remove
...
HABroker.prototype.getDisco = function() {
