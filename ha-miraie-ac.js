...
const onMiraieStateChanged = (topic, payload) => {
    ...
    if (!device) return;
    if(global.mynode) {
      //global.mynode.send(payload.toString())
/*       global.mynode.send({
		payload: payload.toString(),
		friendlyName: device.friendlyName
     }) */
	 global.mynode.send({... JSON.parse(payload.toString()), friendlyName: device.friendlyName})
    }
...
module.exports = function (RED) {
    function MirAIeNode(config) {
        ...
        settings.publishDiscovery = config.publishDiscovery;
                ...
                homeDetails.devices.map(d => {
                    ...
                    .then(function(status) {
						if(config.publishDiscovery) {
						  publishDeviceStatus(status.data, status.device)
						}
						//console.log('status', status)
						//let output = haBroker.getDisco()
						//Logger.logDebug('output => ' + JSON.stringify(output));
						//Logger.logDebug('status data => ' + JSON.stringify(status.data));
						//console.log('getDisco output', output)
						//node.send(status.data)
						node.send({... status.data, friendlyName: d.friendlyName})
						//node.send({data: status.data, d: {friendlyName: d.friendlyName}})
					})
                    .catch(e => {
